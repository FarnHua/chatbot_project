...
class SoftEmbedding(nn.Module):
    def __init__(self,
        ...
        self.learned_embedding = nn.parameter.Parameter(state_dict['model.shared.learned_embedding'])
...
def main():
    ...
    parser.add_argument('--model', type=str,
                    help='pretrained model name or path to local checkpoint', default='gpt2')
...
#     eos = [tokenizer.encoder["<|endoftext|>"]]
    eos = [2]
    n_tokens = args.len
    s = post_set('data/test.tsv', tokenizer = tokenizer, n_tokens = args.len)
    train_dataloader = DataLoader(s, batch_size=4, shuffle=False, num_workers=2)
    with torch.no_grad():
...
#                 prev_input = torch.LongTensor([[eos] * inputs_id.shape[0]]).to(device)
                ...
                bos = [1]
                eos = [2] ## [50256]
                decoder_input_ids = torch.LongTensor([[bos] * inputs_id.shape[0]]).squeeze(0).to(device)
                prev_input = torch.LongTensor([[bos] * inputs_id.shape[0]]).squeeze(0).to(device)
                past = None
...
#                     if args.fil:
#                         logits = top_filtering(logits, top_k=top_k, top_p=top_p)
                    logits = sampling(logits, top_k = 40, temperature = 2.2)
#                     logits = torch.softmax(logits, dim=-1)
                    ...
                    for j in range(0, inputs_id.shape[0]):
                        if temp_sen[j][-1] != eos[0]: 
                            ...
                            temp_sen[j].append(prev_input[j].item())
                    if flag == 1: break
                a = tokenizer.batch_decode(temp_sen, skip_special_tokens=True)
                fir = tokenizer.batch_decode(inputs_id[:, n_tokens:], skip_special_tokens=True)
#                 for j in range(inputs_id.shape[0]):
#                     l = ll[j]
...
#                 print(k)
                ...
                for i in range(inputs_id.shape[0]):
                    NEXT = fir[i][1:] + '</s> <s>' + a[i]
                    inter_reply = model_bot.generate(torch.tensor([tokenizer.encode(NEXT)]).to(device), max_new_tokens = 120, min_length = 1)
                    k.append(tokenizer.batch_decode(inter_reply, skip_special_tokens=True)[0])
            #elif args.inter == 'google':
                #k = []
                # for j in range(inputs_id.shape[0]):
