...
class SoftEmbedding(nn.Module):
    ...
    def initialize_embedding(self, 
        ...
        return torch.FloatTensor(n_tokens, 80).uniform_(-random_range, random_range)
...
def main():
    ...
    parameters_mlp = list(s_wte.mlp.parameters())
    for x in parameters_mlp:
    ...
    # assert(0)
    wandb.config.update({'total_update_param': sum(p.numel() for p in s_wte.mlp.parameters() if p.requires_grad)})
    optimizer = Adam(s_wte.mlp.parameters(), config.lr,
                     max_grad_norm=1.0)
    ##
