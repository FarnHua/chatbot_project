...
from prefixmodel import PrefixTuning
...
def train(model_train, inputs_id, mask, model_2, model_bot, tokenizer, ll, args, batch_size, n_tokens,  batch, reward):
    ...
    coh_score = 0
    for i in range(40): # 40 word
        output = model_train(prev_input, past_key_values=past, attention_mask=mask)
...
def main():
    ...
    model_train = PrefixTuning(device=device_0) 
    ...
    optimizer = Adam(model_train.parameters(), config.lr,
                     max_grad_norm=1.0)
    ##
    ...
    print('Use ', train_para / total_para, ' parameters for training\n')
